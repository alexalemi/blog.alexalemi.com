...
Let's imagine while we are concerned we might have had the disease, but if you are being honest, 
you almost certainly didn't have it,<sup><a href="#kent">xx</a></sup>
...
that if you had been exposed to the disease, the sensitivity or *true positive rate* of the
...
$$ I[p;q] = \int dx\, q(x) f\left( \frac{p(x)}{q(x)} \right). $$

<aside> <sup id="caveat">3</sup>
  We could have just as well written it as $I[p;q] = \int dx\, p(x) g\left( \frac{p(x)}{q(x)} \right)$ (that is, the form
...
what out our new beliefs be?  If we search for the joint distribution $p(x,\theta)$ that is
...
$$ q(\theta, D) = q(\theta) q(D|\theta) = q(D) q(\theta|D) $$
implies
...
The nice thing about measuring information in decibels is first that people already have some familiarity with
the unit (a difference of 10 dB being a factor of 10, 3 dB being a factor of 2),
and secondly that it seems like 1 dB roughly corresponds the smallest detectable value that people
...
<figure id="bigspin">
  <center>
   ...
   <br />
   <input value=0 type='number' style="width: 4em" id="percent" onchange="fraction(11, document.getElementById('percent').value);">
   <label for="percent">dB</label>
   <br/>
   <input id="slider" style="width: 65%;" type="range" min="-30" step="0.1" max="30" value="0" class="slider" id="slider"
...
difference of likelihoods as a form of model selection. Connection to number
of samples and stuff.


### Coding Cost

Encoding a sample from $p$ according to a code designed for $q$ costs the excess bits.
