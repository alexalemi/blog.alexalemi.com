...
Diffusion models have made quite a splash, especially after the open source release of [Stable Diffusion](https://huggingface.co/spaces/stabilityai/stable-diffusion).  What are diffusion models, where does the loss come from and what does a simple example look like? I've recently open-sourced a simple, pedagogical, self-contained 
[example colab](https://colab.research.google.com/github/google-research/vdm/blob/main/colab/SimpleDiffusionColab.ipynb)
of a diffusion model trained on EMNIST, which you can find as part of the [Variational Diffusion Models (VDM)](https://arxiv.org/abs/2107.00630) [github page](https://github.com/google-research/vdm).  In this post, I wanted to give some more background about where the loss function comes from, and what I feel is a simple way to motivate it.

## Non-negativity of KL

<aside><sup id="#p-and-q">1</sup>
 I tend to reverse the use of $p$ and $q$ with respect to the rest of the world.  Most people use $p$ for the generative model and $q$ for the approximate posterior.  They do this because, for most people, the generative model is the star of the show and the approximate posterior is playing second fiddle.  My reversal of the letters is deliberate.  To me, the <i>forward process</i> $p(x,z)=p(x)p(z|x)$ composed of the <i>true image distribution</i> $p(x)$ and the <i>encoder</i> $p(z|x)$ is the star of the show.  It's the star of the show because it's the actual joint distribution of things that exist in the real world, it's the distribution we will always be sampling from.
</aside>

Let's say we want to build a latent-variable model, $q(x, z)$ where the likelihood of data $p(x)$, has high marginal likelihood: $\log q(x)$. Unfortunately, computing $\log q(x)$ involves an intractable integral over the latent variable, $z$.<sup><a href="#p-and-q">1</a></sup>
...
<aside><sup id="#brakets">3</sup>
	I use braket's to show expectations and unless noted, always with respect to the full $p$ distribution.
...
<aside><sup id="#always-joint-p">4</sup>
...
In particular, if we marginalize out some subset of random variables the KL divergence of the marginal distributions has to be less. So for any two random variables:<sup><a href="#always-joint-p">4</a></sup>
$$ \begin{align}
...
Imagine designing these joint distributions to have different flavors.  Think of $p(x,z)$ as a *forward* process $p(x) p(z|x)$ that takes an image from some natural image distribution $p(x)$ and then encodes it into some representation $z$ with an encoder $p(z|x)$.  This is a fully-fledged joint distribution over the two variables, running the forward process would give us $(x,z)$ pairs, pairs of natural images and their encodings.
Next, imagine a different joint distribution, a *reverse process* $q(x,z)$ that takes some sample from a *prior* $q(z)$ and then runs it through a *decoder* $q(x|z)$ to generate a synthetic image.  This is a generative model of the kind we might be used to building.  This is also a fully-fledged joint distribution that we could sample from, in order to generate $(x,z)$ pairs.  Of course, these generated images $q(x)$ would not be the same as the natural image distribution $p(x)$ unless we were extremely lucky. 
...
So, by minimizing the KL between our forward and reverse process, we can ensure that we make progress towards learning a good generative model of our images.
...
<aside><sup id="#ELBO">5</sup>
	<a href="https://en.wikipedia.org/wiki/Evidence_lower_bound">Evidence Lower BOund</a>
</aside>
We've just derived the ordinary ELBO:<sup><a href="#ELBO">5</a></sup>
...
$$\begin{multline}
	...
	\implies \left\langle \log q(x) \right\rangle \leq \left\langle \log q(x|z) - \log \frac{p(z|x)}{q(z)} \right\rangle
\end{multline}$$

At the end of the day, the hope and a dream we seem to have in doing latent variable modeling is that maybe we will somehow be more successful in learning a reverse $q(z)q(x|z)$ process to match some forward $p(x)p(z|x)$ than we would have been able to just model the density $q(x)$ directly.  Somehow we are hoping that by expanding the problem, making it a harder or larger modeling task, it'll somehow become easier for us to actually optimize or learn.
...
<aside><sup id="#variance-preserving">6</sup>
In a lot of the diffusion work, the process is taken to be *variance preserving* by setting:
$$ \alpha^2 = 1 - \sigma^2 $$
</aside>

This takes an ordinary image and then adds more and more noise to it until it looks more or less indistinguishable from just isotropic Gaussian noise.<sup><a href="#variance-preserving">6</a></sup>
...
<figure id="#forward-diffusion">
  ...
  <figcaption>
  Figure 1. A demonstration of the typical forward process in diffusion models.
  </figcaption>
  </center>
</figure>
...
<aside><sup id="#extra-entropy">7</sup>
	Aside, again, from the constant entropy of the data outside our control which we can ignore for purposes of optimization.	
</aside>

The VDM loss is<sup><a href="#extra-entropy">7</a></sup> simply the KL between these two joints, which serves as an upper bound on the KL of the image marginals:
...
<aside><sup id="#deep-unsupervised">8</sup>
...
To utilize our knowledge of the forward process, we're actually going to rewrite the forward process not as a sequence of conditional gaussian steps:
$$ p(x,z_0,z_1,z_2,\cdots,z_T) = p(x) p(z_0|x) p(z_1|z_0) p(z_2|z_1) \cdots p(z_T|z_{T-1}) $$
but instead we'll rearrange this to be a product of a bunch of conditional reverse steps:
$$ 
\begin{align}
